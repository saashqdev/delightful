...
use App\Domain\Chat\Event\Agent\Agentexecute Interface;
...
use Delightful\BeDelightful\Domain\SuperAgent\Repository\Persistence\TokenUsagerecord Repository;
...
use Delightful\BeDelightful\Infrastructure\ExternalAPI\SandboxOS\Asrrecord er\Asrrecord erInterface;
use Delightful\BeDelightful\Infrastructure\ExternalAPI\SandboxOS\Asrrecord er\Asrrecord erService;
...
use Delightful\BeDelightful\list ener\AddRoutelist ener;
use Delightful\BeDelightful\list ener\I18nLoadlist ener;
...
{
 define('SUPER_MAGIC_MODULE_PATH', BASE_PATH . '/vendor/dtyq/super-magic-module'); $publishConfigs = []; // publish/route Folderunder AllFile $routeDir = __DIR__ . '/../publish/route/'; if (is_dir($routeDir)) 
{
 $iterator = new RecursiveIteratorIterator( new RecursiveDirectoryIterator($routeDir, RecursiveDirectoryIterator::SKIP_DOTS), RecursiveIteratorIterator::SELF_FIRST ); foreach ($iterator as $file) 
{
 if ($file->isFile()) 
{
 $relativePath = $file->getSubPath() . '/' . $file->getFilename(); $publishConfigs[] = [ 'id' => 'route_' . str_replace('/', '_', $relativePath), 'description' => 'Route file: ' . $relativePath, 'source' => $file->getPathname(), 'destination' => BASE_PATH . '/config/' . $relativePath, ]; 
}
 
}
 
}
 return [ 'dependencies_priority' => [ // execute Event Agentexecute Interface::class => SuperAgentMessageSubscriberV2::class, SuperAgentMessageInterface::class => SuperAgentMessage::class, ], 'dependencies' => [ // AddInterfaceImplementationClassMap TaskFileRepositoryInterface::class => TaskFileRepository::class, TaskFileCleanupRepositoryInterface::class => TaskFileCleanupRepository::class, TaskFileVersionRepositoryInterface::class => TaskFileVersionRepository::class, TopicRepositoryInterface::class => TopicRepository::class, TaskRepositoryInterface::class => TaskRepository::class, WorkspaceRepositoryInterface::class => WorkspaceRepository::class, TaskMessageRepositoryInterface::class => TaskMessageRepository::class, ProjectRepositoryInterface::class => ProjectRepository::class, ProjectOperationLogRepositoryInterface::class => ProjectOperationLogRepository::class, ProjectOperationLogDomainService::class => ProjectOperationLogDomainService::class, SandboxInterface::class => SandboxService::class, ProjectMemberRepositoryInterface::class => ProjectMemberRepository::class, ProjectMemberSettingRepositoryInterface::class => ProjectMemberSettingRepository::class, // AddSandboxOSrelated ServiceDependency SandboxGatewayInterface::class => SandboxGatewayService::class, SandboxAgentInterface::class => SandboxAgentService::class, FileConverterInterface::class => FileConverterService::class, Asrrecord erInterface::class => Asrrecord erService::class, AgentAppService::class => AgentAppService::class, // AddFileprocess AppServiceDependency Fileprocess AppService::class => Fileprocess AppService::class, // AddHandleAgentMessageAppServiceDependency HandleAgentMessageAppService::class => HandleAgentMessageAppService::class, // AddMessageQueueAppServiceDependency MessageQueueAppService::class => MessageQueueAppService::class, // AddMessageScheduleAppServiceDependency MessageScheduleAppService::class => MessageScheduleAppService::class, // AddSharerelated Service ShareableResourceFactory::class => ShareableResourceFactory::class, TopicShareableResource::class => TopicShareableResource::class, ResourceShareRepositoryInterface::class => ResourceShareRepository::class, ResourceShareAppService::class => ResourceShareAppService::class, TokenUsagerecord RepositoryInterface::class => TokenUsagerecord Repository::class, WorkspaceVersionRepositoryInterface::class => WorkspaceVersionRepository::class, ProjectForkRepositoryInterface::class => ProjectForkRepository::class, MessageQueueRepositoryInterface::class => MessageQueueRepository::class, MessageScheduleLogRepositoryInterface::class => MessageScheduleLogRepository::class, MessageScheduleRepositoryInterface::class => MessageScheduleRepository::class, // agent BeDelightfulAgentRepositoryInterface::class => BeDelightfulAgentRepository::class, TaskFileVersionDomainService::class => TaskFileVersionDomainService::class, MessageScheduleDomainService::class => MessageScheduleDomainService::class, ], 'listeners' => [ AddRoutelist ener::class, I18nLoadlist ener::class, ProjectOperatorLogSubscriber::class, ], 'error_message' => [ 'error_code_mapper' => [ SuperAgentErrorCode::class => [51000, 51299], ShareErrorCode::class => [51300, 51400], ], ], 'commands' => [], 'annotations' => [ 'scan' => [ 'paths' => [ __DIR__, ], ], ], 'publish' => $publishConfigs, ]; 
}
 
    public function getRoutes(): array 
{
 return [ 'routes' => [ 'path' => __DIR__ . '/../publish/route', ], ]; 
}
