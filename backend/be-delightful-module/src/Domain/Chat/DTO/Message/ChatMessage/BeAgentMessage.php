...
{
 protected ?string $topicId = ''; protected ?string $messageId = null; protected ?string $taskId = null; protected ?string $type = null; protected ?string $status = null; protected ?string $content = null; protected ?array $steps = null; protected ?string $event = ''; protected ?string $role = ''; protected ?SuperAgenttool $tool = null; protected ?int $sendTimestamp = null; protected ?array $attachments = null; protected ?string $remark = ''; protected ?MemoryOperation $memoryOperation; protected ?string $correlationId = null; // 🎯 Add correlation_id Field protected ?array $usage = null; // Usage information (only set when task is finished) 
    public function __construct(?array $messageStruct = null) 
{
 parent::__construct(); if ($messageStruct !== null) 
{
...
{
 $decoded = json_decode($tool, true); $this->tool = new SuperAgenttool ($decoded); 
}
 else 
{
 // IfIs notValid JSON StringCanSelectThrowExceptionor Ignore $this->tool = null; 
}
...
{
 $this->sendTimestamp = $sendTimestamp; return $this; 
}
...
{
 $data = array_merge(parent::toArray($filterNull), [ 'topic_id' => $this->topicId, 'message_id' => $this->messageId, 'task_id' => $this->taskId, 'type' => $this->type, 'status' => $this->status, 'content' => $this->content, 'event' => $this->event, 'steps' => array_map(function ($step) 
{
...
, $this->steps ?? []), 'tool' => $this->tool?->toArray(), 'role' => $this->role, 'send_timestamp' => $this->sendTimestamp ?? time(), 'attachments' => $this->attachments, 'remark' => $this->remark, 'correlation_id' => $this->correlationId, 'usage' => $this->usage, ]); if ($filterNull) 
{
...
{
 $decoded = json_decode($memoryOperation, true); $this->memoryOperation = new MemoryOperation($decoded); 
}
 else 
{
 // IfIs notValid JSON StringCanSelectThrowExceptionor Ignore $this->memoryOperation = null; 
}
