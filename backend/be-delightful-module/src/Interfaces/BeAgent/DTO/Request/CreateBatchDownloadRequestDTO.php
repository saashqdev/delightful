...
class CreateBatchDownloadRequestDTO 
{
 /** * @var array File ID array */ 
    private array $fileIds = []; /** * @var string Project ID */ 
    private string $projectId = ''; /** * Get file ID array. */ 
...
{
 $this->fileIds = $fileIds; return $this; 
}
 /** * Get project ID. */ 
    ...
    public function setProjectId(string $projectId): self 
{
 ...
 /** * Create DTO from request data. * * @param array $requestData Request data */ 
    public 
    static function fromRequest(array $requestData): self 
{
 $dto = new self(); $fileIds = $requestData['file_ids'] ?? []; $projectId = $requestData['project_id'] ?? ''; // Validate for project_id if (! is_string($projectId)) 
{
 ExceptionBuilder::throw(SuperAgentErrorCode::VALIDATE_FAILED); 
}
 // Validate for file_ids if (! is_array($fileIds)) 
{
 ExceptionBuilder::throw(SuperAgentErrorCode::BATCH_FILE_IDS_INVALID); 
}
 // Either file_ids or topic_id must be provided if (empty($fileIds) && empty($projectId)) 
{
 ExceptionBuilder::throw(SuperAgentErrorCode::BATCH_FILE_IDS_OR_TOPIC_ID_REQUIRED); 
}
 // If file_ids is provided, validate it if (! empty($fileIds)) 
{
 ...
 if (empty($fileId) || ! is_string($fileId)) 
{
 ...
 $dto->setFileIds($fileIds); $dto->setProjectId($projectId); return $dto; 
}
