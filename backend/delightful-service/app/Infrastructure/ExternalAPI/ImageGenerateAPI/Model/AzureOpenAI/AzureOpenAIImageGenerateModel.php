...
class AzureOpenAIImageGenerateModel extends AbstractImageGenerate
{
    ...
    public function generateImageOpenAIFormat(ImageGenerateRequest $imageGenerateRequest): OpenAIFormatResponse
    {
        ...
        try {
            // 3. 图像generate（同步handle，Azure OpenAI API 支持 n parameter一次性generate多张图片）
    ...
    private function buildResponse(array $result): ImageGenerateResponse
    {
        try {
            ...
            $this->logger->error('Azure OpenAI图像generate：构建响应fail', [
                'error' => $e->getMessage(),
                'result' => $result,
            ]);
