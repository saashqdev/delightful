...
class FragmentPreviewRequestDTO extends AbstractRequestDTO
{
    ...
    public static function getHyperfValidationMessage(): array
    {
        return [
            ...
            'fragment_config.normal.text_preprocess_rule.*.integer' => '预处理规则must是整数',
            'fragment_config.normal.text_preprocess_rule.*.in' => '预处理规则must是 1(替换空格等) 或 2(deleteURL和邮箱)',
            ...
            'fragment_config.parent_child.text_preprocess_rule.*.integer' => '预处理规则must是整数',
            'fragment_config.parent_child.text_preprocess_rule.*.in' => '预处理规则must是 1(替换空格等) 或 2(deleteURL和邮箱)',
        ];
    }
