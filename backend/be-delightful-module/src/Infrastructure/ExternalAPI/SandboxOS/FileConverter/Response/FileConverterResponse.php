...
{
 $this->success = $success; $this->code = $code; $this->message = $message; $this->data = ConverterDataDTO::fromArray($data); 
}
 
    public 
    static function fromGatewayResult(GatewayResult $result): self 
{
 return new self( $result->isSuccess(), $result->getCode(), $result->getMessage(), $result->getData() ); 
}
 
    public 
    static function fromApiResponse(array $response): self 
{
 return new self( ($response['code'] ?? -1) === 1000, $response['code'] ?? -1, $response['message'] ?? 'Unknown error', $response['data'] ?? [] ); 
}
...
{
 return array_map( fn (FileItemDTO $file) => $file->ossKey, $this->getConvertedFiles() ); 
}
 /** * GetFile. */ 
