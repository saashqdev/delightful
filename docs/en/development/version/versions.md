...
Delightful adopts the x.y.z version numbering rule to name each version, such as version 1.2.3, where 1 is x, 2 is y, and 3 is z. You can use this versioning rule to plan your updates to the Delightful project.
- x represents a major version. When Delightful's core undergoes extensive refactoring changes, or when there are numerous breaking API/UI changes, it will be released as an x version. Changes in x versions typically cannot be compatible with previous x versions, although this doesn't necessarily mean complete incompatibility. Specific compatibility should be determined based on the upgrade guide for the corresponding version.
...
When you want to upgrade Delightful versions, if you are upgrading x and y versions, please follow the upgrade guide for the corresponding version in the documentation. 