...
class VolcengineArkModel extends AbstractImageGenerate
{
    ...
    protected function generateImageInternal(ImageGenerateRequest $imageGenerateRequest): ImageGenerateResponse
    {
        ...
        if (empty($imageData)) {
            $this->logger->error('VolcengineArk文生图：所有图片generate均fail', ['rawResults' => $rawResults]);
    ...
    protected function requestImageGeneration(VolcengineArkRequest $imageGenerateRequest): array
    {
        ...
        try {
            ...
            $this->logger->warning('VolcengineArk图片generate：call图片generate接口fail', ['error' => $e->getMessage()]);
            ...
            foreach ($volcengineResult['data'] as $item) {
                if (! empty($item['url'])) {
                    ...
                    try {
                        ...
                        $this->logger->error('VolcengineArk添加图片数据：水印handlefail', [
                            'error' => $e->getMessage(),
                            'url' => $item['url'],
                        ]);
    ...
    private function generateImageRawInternal(ImageGenerateRequest $imageGenerateRequest): array
    {
        ...
        for ($i = 0; $i < $count; ++$i) {
            $parallel->add(function () use ($imageGenerateRequest, $i, $fromCoroutineId) {
                ...
                try {
                    ...
                    $this->logger->error('VolcengineArk文生图：图片generatefail', [
                        'error' => $e->getMessage(),
                        'index' => $i,
                    ]);
                    return [
        ...
        if (empty($rawResults)) {
            ...
            $this->logger->error('VolcengineArk文生图：所有图片generate均fail', ['errors' => $errors]);
    ...
    private function processVolcengineArkRawDataWithWatermark(array $rawData, ImageGenerateRequest $imageGenerateRequest): array
    {
        foreach ($rawData as $index => &$result) {
            ...
            try {
                ...
                $this->logger->error('VolcengineArk图片水印handlefail', [
                    'index' => $index,
                    'error' => $e->getMessage(),
                ]);
            }
