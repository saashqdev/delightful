...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        Schema::create('service_provider_models_config_versions', function (Blueprint $table) {
            ...
            $table->boolean('support_function')->default(false)->comment('是否supportfunctioncall');
            $table->decimal('cache_hit_pricing', 10, 4)->nullable()->comment('cache命中定价');
            $table->integer('max_output_tokens')->nullable()->comment('最大outputtoken数');
            $table->boolean('support_embedding')->default(false)->comment('是否support嵌入');
            $table->boolean('support_deep_think')->default(false)->comment('是否support深度思考');
            $table->decimal('cache_write_pricing', 10, 4)->nullable()->comment('cachewrite定价');
            $table->boolean('support_multi_modal')->default(false)->comment('是否support多模态');
            ...
            $table->decimal('cache_hit_cost', 10, 4)->nullable()->comment('cache命中cost');
