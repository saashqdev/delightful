...
class ProjectArchitectureIntegrityValidator
{
    ...
    {
        try {
            $tableExists = Db::select("SHOW TABLES LIKE 'delightful_be_agent_project'");
            ...
            $columns = Db::select('DESCRIBE delightful_be_agent_project');
            $requiredColumns = [
    ...
    private function validateTopicProjectAssociations(): void
    {
        try {
            ...
            $topicsWithoutProject = Db::select(
                'SELECT COUNT(*) as count FROM delightful_be_agent_topics WHERE project_id = 0 OR project_id IS NULL'
            );
            ...
            $invalidProjectRefs = Db::select('
                ...
                FROM delightful_be_agent_topics t 
                LEFT JOIN delightful_be_agent_project p ON t.project_id = p.id 
    ...
    private function validateProjectWorkspaceConsistency(): void
    {
        try {
            ...
            $invalidWorkspaceRefs = Db::select('
                SELECT COUNT(*) as count 
                FROM delightful_be_agent_project p 
                LEFT JOIN delightful_be_agent_workspaces w ON p.workspace_id = w.id 
                WHERE w.id IS NULL
            ');
            ...
            $userMismatches = Db::select('
                SELECT COUNT(*) as count 
                FROM delightful_be_agent_project p 
                JOIN delightful_be_agent_workspaces w ON p.workspace_id = w.id 
    ...
    private function validateDefaultProjectCreation(): void
    {
        try {
            $workspacesWithoutProjects = Db::select('
                SELECT COUNT(*) as count 
                FROM delightful_be_agent_workspaces w 
                LEFT JOIN delightful_be_agent_project p ON w.id = p.workspace_id 
    ...
    private function validateFileTableRename(): void
    {
        try {
            ...
            $oldTableExists = Db::select("SHOW TABLES LIKE 'delightful_be_agent_task_files'");
            $newTableExists = Db::select("SHOW TABLES LIKE 'delightful_be_agent_project_files'");
            ...
            if (empty($oldTableExists) && ! empty($newTableExists)) {
                ...
                $columns = Db::select('DESCRIBE delightful_be_agent_project_files');
    ...
    private function validateProjectStatusValues(): void
    {
        try {
            $invalidStatuses = Db::select("
                SELECT COUNT(*) as count 
                FROM delightful_be_agent_project 
