...
class CreateDelightfulChatMessageTable extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('delightful_chat_messages', static function (Blueprint $table) {
            ...
            $table->string('receive_id', 64)->comment('receive方id，可能是人类、ai或者应用/文档/多维table格等');
            $table->tinyInteger('receive_type')->comment('receive方type,1:user(ai也被认为是user)；2：应用;3:文档;4:多维table格');
            $table->string('receive_organization_code', 64)->comment('receive方organization编码,可能为空string')->default('');
            // message的相关id
            ...
            $table->string('message_type', 32)->comment('messagetype:文本/table情/file/markdown等');
