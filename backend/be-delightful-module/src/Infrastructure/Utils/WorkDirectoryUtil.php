...
class WorkDirectoryUtil
{
    ...
    public static function isValidWorkDirectoryLegacy(string $workDir, string $userId): bool
    {
        ...
        // Check if it contains the expected pattern: SUPER_DELIGHTFUL/{userId}/project_{projectId}[/workspace]
        ...
        $pattern = sprintf(
            ...
            preg_quote(AgentConstant::SUPER_DELIGHTFUL_CODE, '/'),
            preg_quote(AgentConstant::SUPER_DELIGHTFUL_CODE, '/'),
            preg_quote($userId, '/')
        );
    ...
    public static function extractProjectIdFromAbsolutePathLegacy(string $workDir, string $userId): ?string
    {
        ...
        // Expected format: path/to/SUPER_DELIGHTFUL/{userId}/project_{projectId}[/workspace]
        // Supports both legacy format (project_id only) and new format (with /workspace suffix)
        // We need to find the pattern: SUPER_DELIGHTFUL/{specificUserId}/project_{projectId}
        // The pattern should work for both relative and absolute paths
        $pattern = sprintf(
            '/(?:.*\/%s|^%s)\/%s\/project_(\d+)(?:\/workspace)?$/',
            preg_quote(AgentConstant::SUPER_DELIGHTFUL_CODE, '/'),
    ...
    {
        return sprintf('be-delightful:workspace:locker:%d', $projectId);
    }
