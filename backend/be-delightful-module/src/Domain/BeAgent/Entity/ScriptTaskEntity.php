...
{
 // DefaultSet parent::__construct($data); 
}
 /** * Convert toArray. */ 
    public function toArray(): array 
{
 $result = [ 'task_id' => $this->taskId, 'arguments' => $this->arguments, 'script_name' => $this->scriptName, ]; // RemoveNull return array_filter($result, function ($value) 
{
 return ! empty($value); 
}
); 
...
{
 return new self([ 'task_id' => $data['task_id'] ?? $data['taskId'] ?? '', 'arguments' => $data['arguments'] ?? '', 'script_name' => $data['script_name'] ?? $data['scriptName'] ?? '', ]); 
}
