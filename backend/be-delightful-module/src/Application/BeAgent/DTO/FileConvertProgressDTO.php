...
{
 // IfFirstParameteryes ArrayUsingArrayInitialize if (is_array($data)) 
{
 parent::__construct($data); return; 
}
 // CompatibleIfyes SingleParameterHaveprocess if ($data !== null) 
{
...
{
 return new self( $data['current'] ?? 0, $data['total'] ?? 0, $data['percentage'] ?? 0.0, $data['message'] ?? '' ); 
}
 // ====== Getters ====== 
...
{
 $this->current = $current; return $this; 
}
