...
export const visiblePorts: any[] = [] // Track page visibility states

// Wrap intervalId in an object to make it mutable
export const state = {
	intervalId: null as NodeJS.Timeout | null,
}

// Send message to all windows except the sender
	...
	port.onmessage = async function onmessage(e: any) {
		...
		switch (type) {
			...
			case MessageType.STOP: // Stop polling
				...
				// Stop polling only when all pages are not visible
			...
			case MessageType.CLOSE: // Close current port
				...
				break
			case MessageType.REFRESH: // Actively refresh and notify other pages
				sendMessage(port, {
