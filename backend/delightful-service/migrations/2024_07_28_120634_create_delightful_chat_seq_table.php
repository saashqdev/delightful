...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        Schema::create('delightful_chat_sequences', static function (Blueprint $table) {
            ...
            $table->string('object_id', 64)->comment('objectid. if是user时,table示delightful_id');
            $table->string('seq_id', 64)->comment('message序列号 id，each个账号的所havemessagemust逐渐增大');
            $table->string('seq_type', 32)->comment('message大type:控制message,chatmessage。');
            $table->text('content')->comment('序列号detail. 一些not可见的控制message,只inseqtable存indetail. by及写时复制一份messagetablecontenttoseqtableuse.');
            $table->string('delightful_message_id', 64)->comment('service端generate的唯一messageid,useatmessagewithdraw/edit');
            $table->string('message_id', 64)->comment('序列号associate的usermessageid,implement已读回执,messagewithdraw/editetc')->default(0);
            // quote的messageid
            ...
            // messagereceive人list
            $table->text('receive_list')->comment('messagereceive人list,all量record未读/已读/已查看userlist');
            ...
            // by下索引create移动to单独的迁移file中
            $table->timestamps();
            $table->softDeletes();
            $table->comment('账号收件箱的序列号table,each个账号的所havemessagemust单调递增');
        });
    }
