...
class UserSearchNodeRunner extends AbstractSearchNodeRunner
{
        ...
        if (! empty($allUserIds)) {
            ...
            foreach ($delightfulUsers as $delightfulUser) {
                ...
                $users[] = [
                    ...
                    'real_name' => $delightfulAccount->getRealName(),
                    'position' => $userDepartment[0]['job_title'] ?? '',
                    'phone_number' => $delightfulAccount->getCountryCode() . $delightfulAccount->getPhone($phoneDesensitization),
    ...
    // -------- 以下method十分粗暴，不建议学习 🔞🈲 --------  todo 等 es 或 flink cdc 之类的出来再优化
    ...
    private function getUserIdsByUsername(Operator $operator, OperatorType $operatorType, mixed $username, ?array $filterUserIds = null): array
    {
        ...
        $db = AccountModel::query()->where('type', '=', 1);
        switch ($operatorType) {
    ...
    private function getUserIdsByWorkNumber(Operator $operator, OperatorType $operatorType, mixed $workNumber, ?array $filterUserIds = null): array
    {
        ...
        $db = DepartmentUserModel::query()->where('organization_code', '=', $operator->getOrganizationCode());
        switch ($operatorType) {
            case OperatorType::Equals:
    ...
    {
        $db = DepartmentModel::query()->where('organization_code', '=', $operator->getOrganizationCode());
        switch ($operatorType) {
            case OperatorType::Equals:
