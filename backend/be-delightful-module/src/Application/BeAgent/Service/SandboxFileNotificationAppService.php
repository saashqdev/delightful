...
use BeDelightful\BeDelightful\Interfaces\BeAgent\DTO\Request\SandboxFileNotificationRequestDTO;
...
class SandboxFileNotificationAppService extends AbstractAppService
{
    ...
    private function getProjectEntity(MessageMetadata $metadata)
    {
        ...
        if (! $taskEntity) {
            ExceptionBuilder::throw(BeAgentErrorCode::TASK_NOT_FOUND, 'Task not found');
        }

        $projectId = $taskEntity->getProjectId();
        if (empty($projectId)) {
            ExceptionBuilder::throw(BeAgentErrorCode::PROJECT_NOT_FOUND, 'Project ID not found in metadata');
        }
