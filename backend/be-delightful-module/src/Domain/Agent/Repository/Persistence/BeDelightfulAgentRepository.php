...
{
 $builder = $this->createBuilder($dataIsolation, BeDelightfulAgentModel::query()); /** @var null|BeDelightfulAgentModel $model */ $model = $builder->where('code', $code)->first(); if (! $model) 
{
 return null; 
}
...
{
 $builder = $this->createBuilder($dataIsolation, BeDelightfulAgentModel::query()); if (! is_null($query->getCodes())) 
{
 if (empty($query->getCodes())) 
{
 ...
 if ($query->getcreator Id() !== null) 
{
 $builder->where('creator', $query->getcreator Id()); 
}
 if ($query->getName()) 
{
 ...
 if ($query->getEnabled() !== null) 
{
 ...
 $result = $this->getByPage($builder, $page, $query); $list = []; /** @var BeDelightfulAgentModel $model */ foreach ($result['list'] as $model) 
{
 $entity = BeDelightfulAgentFactory::createEntity($model); $list[] = $entity; 
}
...
{
 $builder = $this->createBuilder($dataIsolation, BeDelightfulAgentModel::query()); $model = $builder->where('id', $entity->getId())->first(); 
}
 $model->fill($this->getAttributes($entity)); $model->save(); $entity->setId($model->id); return $entity; 
}
 
    public function delete(BeDelightfulAgentDataIsolation $dataIsolation, string $code): bool 
{
 $builder = $this->createBuilder($dataIsolation, BeDelightfulAgentModel::query()); return $builder->where('code', $code)->delete() > 0; 
}
 
    public function countBycreator (BeDelightfulAgentDataIsolation $dataIsolation, string $creator): int 
{
 $builder = $this->createBuilder($dataIsolation, BeDelightfulAgentModel::query()); return $builder->where('creator', $creator)->count(); 
}
