...
/** * Save topic request DTO * Used to receive request parameters for creating or updating topic. */

class SaveTopicRequestDTO extends AbstractRequestDTO 
{
 /** * Topic ID, empty means create new topic. */ 
    public string $id = ''; /** * Workspace ID. */ 
    public string $workspaceId = ''; /** * Topic name. */ 
    public string $topicName = ''; /** * Project ID. */ 
    public string $projectId = ''; /** * Project mode. */ 
    public string $projectMode = ''; /** * Topic mode. */ 
    public string $topicMode = ''; /** * Get topic ID (primary key). */ 
...
{
 return [ 'id' => 'nullable|string', 'topic_name' => 'present|string|max:100', 'project_id' => 'required|string', 'project_mode' => 'nullable|string', ]; 
}
 /** * Get custom error messages for validation failures. */ 
    protected 
    static function getHyperfValidate Message(): array 
{
 return [ 'topic_name.present' => 'Topic name field is required', 'topic_name.max' => 'Topic name cannot exceed 100 characters', 'project_id.required' => 'Project ID cannot be empty', 'project_id.string' => 'Project ID must be a string', ]; 
}
