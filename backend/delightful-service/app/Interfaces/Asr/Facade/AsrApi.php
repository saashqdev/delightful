...
class AsrApi extends AbstractApi
{
    ...
    public function getUploadToken(RequestInterface $request): array
    {
        ...
        try {
            ...
            try {
                ...
                // .asr_recordings 目录createfail不影响主process
            ...
            try {
                ...
                // .asr_states 目录createfail不影响主process
            ...
            if (
                ...
                try {
                    ...
                    if (! empty($generatedTitle)) {
                        $this->logger->info('file直传标题generatesuccess', [
                            'task_key' => $taskKey,
                            'file_name' => $fileName,
                    ...
                    // 标题generatefail不影响主process
                    $this->logger->warning('file直传标题generatefail', [
                        'task_key' => $taskKey,
                        'file_name' => $fileName,
                        'error' => $e->getMessage(),
                    ]);
            ...
            if (
                ...
                try {
                    ...
                    // savepresetfileID和路径到taskstatus
    ...
    private function validateAndBuildSummaryRequest(RequestInterface $request, DelightfulUserAuthorization $userAuthorization): SummaryRequestDTO
    {
        ...
        // 1. 尝试从 Redis 中get已generate的标题（file直传场景）
        if (! empty($taskKey)) {
        ...
        // 2. 如果没有从 Redis get到标题，则重新generate（前端录音或旧逻辑）
        if (empty($generatedTitle)) {
