...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::table('delightful_be_agent_message', function (Blueprint $table) {
            ...
            if (! Schema::hasIndex('delightful_be_agent_message', 'idx_topic_task_seq_desc')) {
                Db::statement('CREATE INDEX idx_topic_task_seq_desc ON delightful_be_agent_message (topic_id, task_id, seq_id DESC)');
            }

            // 场景2: topic_id + task_id + sender_type + processing_status
            // 用于查询特定topic和task下指定发送者类型和处理状态的消息
            if (! Schema::hasIndex('delightful_be_agent_message', 'idx_topic_task_sender_status')) {
            ...
            // 检查并删除重复的id索引（主键已经包含）
            if (Schema::hasIndex('delightful_be_agent_message', 'idx_id')) {
                $table->dropIndex('idx_id');
            }

            // 检查并删除重复的message_id索引（后续会删除唯一索引）
            if (Schema::hasIndex('delightful_be_agent_message', 'idx_message_id')) {
                $table->dropIndex('idx_message_id');
            }

            // 检查并删除task_type索引
            if (Schema::hasIndex('delightful_be_agent_message', 'idx_task_type')) {
                $table->dropIndex('idx_task_type');
            }

            // 检查并删除sender_created索引
            if (Schema::hasIndex('delightful_be_agent_message', 'idx_sender_created')) {
                $table->dropIndex('idx_sender_created');
            }

            // 检查并删除receiver_created索引
            if (Schema::hasIndex('delightful_be_agent_message', 'idx_receiver_created')) {
                $table->dropIndex('idx_receiver_created');
            }

            // 请确认业务逻辑中有其他方式保证message_id的唯一性
            if (Schema::hasIndex('delightful_be_agent_message', 'delightful_be_agent_message_message_id_unique')) {
                $table->dropUnique('delightful_be_agent_message_message_id_unique');
            }
