...
class FragmentPreviewRequestDTO extends AbstractRequestDTO
{
    ...
    {
        return [
            'document_file.required' => 'documentfile不能为空',
            'document_file.name.required' => 'documentname不能为空',
            'document_file.name.max' => 'documentnamelength不能超过255个字符',
            'document_file.key.required' => 'document键不能为空',
            'document_file.key.max' => 'document键length不能超过255个字符',
            'fragment_config.required' => '片段configuration不能为空',
            'fragment_config.mode.required' => '分段模式不能为空',
            'fragment_config.mode.integer' => '分段模式must是整数',
            'fragment_config.mode.in' => '分段模式must是 1(通用模式) 或 2(父子分段)',
            'fragment_config.normal.required_if' => '通用模式configuration不能为空',
            'fragment_config.normal.text_preprocess_rule.required_if' => '文本预processrule不能为空',
            'fragment_config.normal.text_preprocess_rule.*.required' => '预processrule项不能为空',
            'fragment_config.normal.text_preprocess_rule.*.integer' => '预processrulemust是整数',
            'fragment_config.normal.text_preprocess_rule.*.in' => '预processrulemust是 1(替换空格等) 或 2(deleteURL和邮箱)',
            'fragment_config.normal.segment_rule.required_if' => '分段rule不能为空',
            'fragment_config.normal.segment_rule.separator.required_if' => '分段标识符不能为空',
            'fragment_config.normal.segment_rule.chunk_size.required_if' => '最大分段length不能为空',
            'fragment_config.normal.segment_rule.chunk_size.min' => '最大分段lengthmustgreater than0',
            'fragment_config.normal.segment_rule.chunk_overlap.required_if' => '分段重叠length不能为空',
            'fragment_config.normal.segment_rule.chunk_overlap.min' => '分段重叠lengthmustgreater thanequal0',
            'fragment_config.parent_child.required_if' => '父子分段configuration不能为空',
            ...
            'fragment_config.parent_child.parent_mode.in' => '父块模式must是 1(段落) 或 2(权威)',
            'fragment_config.parent_child.child_segment_rule.required_if' => '子块分段rule不能为空',
            'fragment_config.parent_child.child_segment_rule.separator.required_if' => '子块分段标识符不能为空',
            'fragment_config.parent_child.child_segment_rule.chunk_size.required_if' => '子块最大分段length不能为空',
            'fragment_config.parent_child.child_segment_rule.chunk_size.min' => '子块最大分段lengthmustgreater than0',
            'fragment_config.parent_child.parent_segment_rule.required_if' => '父块分段rule不能为空',
            'fragment_config.parent_child.parent_segment_rule.separator.required_if' => '父块分段标识符不能为空',
            'fragment_config.parent_child.parent_segment_rule.chunk_size.required_if' => '父块最大分段length不能为空',
            'fragment_config.parent_child.parent_segment_rule.chunk_size.min' => '父块最大分段lengthmustgreater than0',
            'fragment_config.parent_child.text_preprocess_rule.required_if' => '文本预processrule不能为空',
            'fragment_config.parent_child.text_preprocess_rule.*.required' => '预processrule项不能为空',
            'fragment_config.parent_child.text_preprocess_rule.*.integer' => '预processrulemust是整数',
            'fragment_config.parent_child.text_preprocess_rule.*.in' => '预processrulemust是 1(替换空格等) 或 2(deleteURL和邮箱)',
        ];
    }
