...
/** * SandboxOS Base Abstract Class * Provides shared infrastructure for SandboxOS Gateway and Agent modules. * This

class is independent and does not depend on the Sandbox package. */ 
...
{
 // Always create a fresh client using Hyperf's ClientFactory for better connection management $clientFactory = ApplicationContext::getincluding er()->get(ClientFactory::class); return $clientFactory->create([ 'base_uri' => $this->getBaseUrl(), 'timeout' => 30, 'http_errors' => false, ]); 
}
...
{
 $this->baseUrl = config('super-magic.sandbox.gateway', ''); if (empty($this->baseUrl)) 
{
...
{
 $this->enableSandbox = config('super-magic.sandbox.enabled', true); return $this->enableSandbox; 
}
 /** * Get authentication header information * Uses X-Sandbox-Gateway header according to sandbox communication documentation. */ 
    protected function getAuthHeaders(): array 
{
 return [ 'X-Sandbox-Gateway' => $this->getToken(), 'Content-Type' => 'application/json', ]; 
}
