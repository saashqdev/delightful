...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        Schema::create('delightful_contact_accounts', static function (Blueprint $table) {
            ...
            $table->string('delightful_id', 64)->comment('账numberid,跨租户(organization)唯一. 为了避免与user_id(organizationinside唯一)的概念混淆,therefore起名了delightful_id')->default('');
            // 账numbertype
            $table->tinyInteger('type')->comment('账numbertype,0:ai,1:人category')->default(0);
            // ai_code
            $table->string('ai_code', 64)->comment('aiencoding')->default('');
            // 账numberstatus
            $table->tinyInteger('status')->comment('账numberstatus,0:正常,1:disable')->default(0);
            // 国际冠码
            ...
            // property别
            $table->tinyInteger('gender')->comment('property别，0:未知；1:男；2:女')->default(0);
            // attach加property
            ...
            $table->timestamps();
            $table->softDeletes();
            $table->comment('user账numbertable,recorduser跨organization唯一的info,such as的hand机number/真名/property别/usertypeetc');
        });
    }
