...
{
 $this->logger->info('[BeDelightful][App] Save file data', [ 'sandbox_id' => $sandboxId, 'file_data_list' => $fileDatalist , 'work_dir' => $workDir, ]); $files = []; foreach ($fileDatalist as $fileData) 
{
 $files[] = [ 'file_key' => $fileData['file_key'], 'file_path' => WorkDirectoryUtil::getRelativeFilePath($fileData['file_key'], $workDir), 'content' => $fileData['content'], 'is_encrypted' => $fileData['is_encrypted'], ]; 
}
 $request = SaveFilesRequest::create($files); $response = $this->agent->saveFiles($sandboxId, $request); if (! $response->isSuccess()) 
{
 throw new SandboxOperationException( 'Save files via sandbox', $response->getMessage(), $response->getCode() ); 
}
