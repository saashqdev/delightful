...
function SubGroup({ subGroup, getGroupNodeList, materialFn }: SubGroupProps) {
	...
	// track child panel expansionstatus
	const [activeKey, setActiveKey] = useState<string | undefined>(undefined)
	const [isRendered, setIsRendered] = useState(false)
	// cachenodelist，avoid re-rendering every expansionget
	...
	// optimization：useuseCallbackwraprenderItemto avoid unnecessarycreate
	...
	const handleCollapseChange = useCallback(
		(key: string | string[]) => {
			...
			// Ifis first expansion，mark as rendered andloadnodedata
...
// useReact.memowrapcomponentand add custom comparisonfunction
export default React.memo(SubGroup, (prevProps, nextProps) => {
	// only compare keyproperty，reduce unnecessary re-rendering
	return (
