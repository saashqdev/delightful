...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        Schema::create('delightful_chat_conversations', static function (Blueprint $table) {
            ...
            // 收件人organization编码
            $table->tinyInteger('receive_type')->comment('sessiontype。1：私聊，2：群聊，3：系统message，4：云文档，5：多维table格 6：话题 7：应用message');
            $table->string('receive_id', '64')->comment('session另一方的id。不同的conversation type，id含义不同。');
            ...
            // 自定义field
            $table->text('extra')->comment('自定义field')->nullable();
            $table->timestamps();
            $table->softDeletes();
            $table->unique(['user_id', 'receive_id', 'receive_type', 'user_organization_code', 'receive_organization_code'], 'unq_user_conversation');
            $table->comment('user的sessionlist。session可能是私聊、群聊、系统message、一个云文档或者多维table格等。');
        });
    }
