...
/** * ItemFile upload STS Token Request DTO. */

class ProjectUploadTokenRequestDTO implements JsonSerializable 
{
 /** * Project ID. */ 
    private string $projectId = ''; /** * Validseconds . */ 
    private int $expires = 3600; /** * FromRequestDataCreateDTO. */ 
    public 
    static function fromRequest(array $data): self 
{
 $instance = new self(); $instance->projectId = $data['project_id'] ?? ''; $instance->expires = (int) ($data['expires'] ?? 3600); return $instance; 
}
...
{
 return [ 'project_id' => $this->projectId, 'expires' => $this->expires, ]; 
}
