...
namespace Dtyq\BeDelightful\Infrastructure\ExternalAPI\SandboxOS;
...
abstract class AbstractSandboxOS
{
    ...
    protected function getBaseUrl(): string
    {
        if (empty($this->baseUrl)) {
            $this->baseUrl = config('be-delightful.sandbox.gateway', '');
            if (empty($this->baseUrl)) {
    ...
    {
        if (empty($this->token)) {
            $this->token = config('be-delightful.sandbox.token', '');
        }
        return $this->token;
    }
    ...
    {
        $this->enableSandbox = config('be-delightful.sandbox.enabled', true);
