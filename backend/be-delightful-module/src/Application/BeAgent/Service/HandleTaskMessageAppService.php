 ...
 if ($accessToken->getType() !== AccessTokenType::Application->value) 
{
 ...
 return $this->userDomainService->getByuser Id(uid: $uid); 
}
 
    public function getTask(int $taskId): TaskEntity 
{
 $taskEntity = $this->taskDomainService->getTaskById($taskId); if (empty($taskEntity)) 
{
 // ExceptionTaskdoes not exist ExceptionBuilder::throw(SuperAgentErrorCode::TASK_NOT_FOUND, 'task.task_not_found'); 
}
 return $taskEntity; 
}
...
{
 $scriptTaskEntity = new ScriptTaskEntity(); $scriptTaskEntity->setSandboxId($requestDTO->getSandboxId()); $scriptTaskEntity->setTaskId($requestDTO->getTaskId()); $scriptTaskEntity->setScriptName($requestDTO->getScriptName()); $scriptTaskEntity->setArguments($requestDTO->getArguments()); $this->taskDomainService->executeScriptTask($scriptTaskEntity); 
}
 /** * Pre-task detection. */ 
 ...
 // get the current task run count $currentTaskRunCount = $this->pulluser TopicStatus($dataIsolation); $taskRound = $this->taskDomainService->getTaskNumByTopicId($topicEntity->getId()); // get department ids $departmentIds = []; $departmentuser Entities = $this->departmentuser DomainService->getDepartmentuser sByuser Ids([$dataIsolation->getcurrent user Id()], $dataIsolation); foreach ($departmentuser Entities as $departmentuser Entity) 
{
 ...
 AsyncEventUtil::dispatch(new RunTaskBeforeEvent($dataIsolation->getcurrent OrganizationCode(), $dataIsolation->getcurrent user Id(), $topicEntity->getId(), $taskRound, $currentTaskRunCount, $departmentIds, $language, $modelId, '', $prompt, $mentions ?? '')); $this->logger->info(sprintf('Dispatched task start event, topic id: %s, round: %d, currentTaskRunCount: %d (after real status check)', $topicEntity->getId(), $taskRound, $currentTaskRunCount)); 
}
...
{
 // Create sandbox container $fullPrefix = $this->taskFileDomainService->getFullPrefix($dataIsolation->getcurrent OrganizationCode()); $fullWorkdir = WorkDirectoryUtil::getFullWorkdir($fullPrefix, $taskContext->getTask()->getWorkDir()); if (empty($taskContext->getSandboxId())) 
{
 ...
 else 
{
 ...
 $sandboxId = $this->agentDomainService->createSandbox($dataIsolation, (string) $taskContext->getProjectId(), $sandboxId, $fullWorkdir); $taskContext->setSandboxId($sandboxId); // user long term memory $memory = $this->longTermMemoryDomainService->getEffectiveMemoriesForPrompt( $dataIsolation->getcurrent OrganizationCode(), AppCodeEnum::SUPER_MAGIC->value, $dataIsolation->getcurrent user Id(), (string) $taskContext->getProjectId(), ); $projectEntity = $this->projectDomainService->getProjectNotuser Id($taskContext->getTask()->getProjectId()); // Initialize agent $this->agentDomainService->initializeAgent($dataIsolation, $taskContext, $memory, projectOrganizationCode: $projectEntity->getuser OrganizationCode()); // Wait for workspace to be ready $this->agentDomainService->waitForWorkspaceReady($taskContext->getSandboxId()); // Send message to agent $this->agentDomainService->sendChatMessage($dataIsolation, $taskContext); // Send message to agent return $sandboxId; 
}
