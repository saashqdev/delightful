...
declare(strict_types=1);
 ...
 Schema::create('delightful_super_agent_message_scheduled', function (Blueprint $table)  {
 $table->bigInteger('id')->primary()->comment('primary keyID (SnowflakeID)');
 $table->string('user_id', 128)->comment('user ID');
 $table->string('organization_code', 64)->comment('userOrganization code');
 $table->string('task_name', 255)->comment('scheduled taskname');
 $table->string('message_type', 64)->comment('message type');
 $table->json('message_content')->comment('message content');
 $table->bigInteger('workspace_id')->unsigned()->comment('workspace ID');
 $table->bigInteger('project_id')->unsigned()->comment('project ID');
 $table->bigInteger('topic_id')->unsigned()->comment('topic ID');
 $table->tinyInteger('completed')->default(0)->comment('yesnoComplete: 0- un Complete, 1-completed');
 $table->tinyInteger('enabled')->default(1)->comment('whether enabled: 0-disabled, 1-enabled');
 $table->dateTime('deadline')->Nullable()->comment('end time');
 $table->string('remark', 500)->default('')->comment('note');
 $table->json('time_config')->comment('configuration information');
 $table->bigInteger('task_scheduler_crontab_id')->Nullable()->comment('TaskScheduleDevice definedwhentask ID');
